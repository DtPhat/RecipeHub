...
import NoFriends from '../pages/friends/NoFriends'
import SearchBar from './SearchBar'
import Skeleton from './Skeleton'

const SharingModal = ({ id, open, setOpen }) => {
      ...
      <Modal.Body >
        {
          <section className='flex flex-col space-y-4'>
            <div className='space-y-2  px-2 text-accent'>
                ...
                (!friendList.length) ?
                  ...
                  : <div className='flex flex-col gap-2 max-h-96 overflow-auto'>
                    {(searchedFriendList)?.map(friend =>
                      <div key={friend.userId} className={`${emails?.includes(friend.email) ? 'outline outline-green-accent' : 'hover:border-accent'} flex items-center border-2 border-gray rounded space-x-4 py-2 px-4 m-1 bg-container cursor-pointer relative`}
            ...
            <div className='space-y-2 border-b-2 border-accent pb-4 px-2 flex items-center gap-2 flex-col md:flex-row text-accent'>
            ...
            <div className='flex flex-wrap gap-2'>
              <h1 className='text-xl font-semibold text-black dark:text-white'>Share to emails:</h1>
              {emails.map((email) => {
                return (
                  <p key={email} className='border rounded-full py-0.5 px-3 space-x-2 border-green-variant text-accent font-semibold'>
                    <span>{email}</span>
            ...
            <div className='flex gap-4 justify-end pt-2 pr-4'>
              ...
              <button className='button-contained w-32' onClick={() => share()} disabled={submitting || !emails.length}>
                {submitting ?
