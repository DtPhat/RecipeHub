...
const PlannedMeals = ({ chosenDate }) => {
  ...
  const recipesElement = dummyRecipes.map(item => {
    ...
    return (
      <div className='flex border-2 border-green-variant p-2 rounded bg-gray-100 relative cursor-pointer' onClick={()=>{setOpen(true)}}>
        <img src={images[0].imageUrl} alt="" className='min-w-[8rem] h-32 object-fit rounded' />
        ...
        {open && <RecipeDetails innerRef={ref} setOpen={setOpen} recipe={item}/>}
      </div>) 
  })
  return (
